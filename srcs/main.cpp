...
void		updatePollerWithNewConfFile(
				Poller* &poller,
				Json::Value const &configuration,
				std::future<void> &backgroundPoll)
{
	if (poller)
	{
		if(configuration["type"].asString() == "icmp")
		{
		...
		if (boost::filesystem::exists(p))
		{
			...
			if(modificationTime < currentModificationTime)
			{
				...
				if (configuration != Json::nullValue)
					updatePollerWithNewConfFile(poller, configuration, backgroundPoll);
			}
		}
		else
		{
